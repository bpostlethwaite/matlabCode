...
relax = visc/E/(60*60*24*365)

diapirforce = 2*delta_rho*g/(r*pi);

visc2 = c/(1) * exp(q/(RR*250));
risetime = visc2/diapirforce/(60*60*24*365)
%%
%Calculate total ice depth with ratios

Rtgamma2 = [0.0560 0.0520 0.05 0.048];
te = 1000;
Depth = te./Rtgamma2;
power = [2 3 4 5]*1e-7;
NPower = linspace(2e-7,5e-7,100);
PRZ = interp1(power,Depth,NPower,'pchip');
