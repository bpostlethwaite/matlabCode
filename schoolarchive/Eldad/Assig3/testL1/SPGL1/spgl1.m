...
%
% (*)      minimize  ||x||_1  subj to  ||Ax-b||_2 <= sigma,
%
% or the l1-regularized least-squares problem
%
% (**)     minimize  ||Ax-b||_2  subj to  ||x||_1 <= tau.
% ---------------------------------------------------------------------
%
% INPUTS
% ======
...
%   spgl1.m
%   $Id: spgl1.m 384 2007-08-11 18:14:42Z mpf $
%
%   ----------------------------------------------------------------------
...
DATE     = '$Date: 2007-08-11 11:14:42 -0700 (Sat, 11 Aug 2007) $';
...
if nargin < 5, x = []; end
if nargin < 4 || isempty(sigma) || isinf(sigma)
   sigma = 0; singleTau = true;
else
   singleTau = false;
end
if nargin < 3 || isempty(tau),     tau     = 0;  end
...
'iterations' ,    2*m , ... % Max number of iterations
...
aProds = zeros(min(maxIts,10000),1);
...
printf(' SPGL1  v.%s (%s)\n', REVISION, DATE);
printf(' %s\n',repmat('=',1,80));
printf(' %-20s: %8.2e %5s' ,'Initial tau'       ,tau     ,'');
printf(' %-20s: %8.2e\n'   ,'Two-norm of b'     ,bNorm      );
...
while 1
    ...
    % Multiple tau: Check if found root and/or if tau needs updating.
       ...
       testUpdateTau  = ((testRelChange1 && rNorm >  2 * sigma) || ...
                         (testRelChange2 && rNorm <= 2 * sigma)) && ~stat;
    ...
    % Update history info
    xNorm1(iter+1) = norm(x,1);
    rNorm2(iter+1) = rNorm;
    aProds(iter+1) = nProdA + nProdAt;
    lambda(iter+1) = gNorm;
...
function x = project(x, tau)
   tStart      = toc;
   if realx
      x = oneProjector(x, tau);
   else
      xa  = abs(x);
      idx = xa > eps;
      xc  = oneProjector(xa,tau);
      x   = x .* xc;
      x(idx) = x(idx) ./ xa(idx);
   end
...
function [nnzX,nnzG,nnzIdx,nnzDiff] = activeVars(x,g,nnzIdx,optTol)
