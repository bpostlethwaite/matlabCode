...
by = sparse(n1+1,1);   % Add no boundary in for the y and z direction
bz = by;
%}


B3 = u(0) + u(ydx(1,:,:)) + u(zdx(1,:,:));
...
By = kron(ones(n3,1),kron(by,ones(n2,1)));
Bz = kron(bz,kron(ones(n2,1),ones(n1,1)));

mBx = kron(ones(n3,1),kron(ones(n2,1),mbx));
mBy = kron(ones(n3,1),kron(mby,ones(n2,1)));
...
%% Solve
    ...
    Jzero = zeros(numel(J(xdx)),1);  
    Jvec = [J(xdx(:)) ; Jzero(:) ; Jzero(:) ];
